  ...
  setup(__props) {
    ...
    const fetchUserInfo = async () => {
      try {
        ...
        common_vendor.index.__f__("error", "at pages/profile/edit-profile.vue:177", "获取用户信息错误:", error);
        common_vendor.index.showToast({
          title: "网络错误，请稍后重试",
          icon: "none"
        });
    ...
    const saveProfile = async () => {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pages/profile/edit-profile.vue:251", "保存个人资料错误:", error);
        common_vendor.index.showToast({
          title: "网络错误，请稍后重试",
          icon: "none"
        });
    ...
    const uploadAvatar = async (tempFilePath) => {
      ...
      try {
        ...
        const uploadTask = common_vendor.index.uploadFile({
          ...
          success: async function(uploadRes) {
            try {
              ...
              common_vendor.index.__f__("error", "at pages/profile/edit-profile.vue:322", "解析上传响应错误:", error);
              common_vendor.index.showToast({
                title: "上传失败，请稍后重试",
                icon: "none"
              });
          ...
          fail: function(error) {
            common_vendor.index.__f__("error", "at pages/profile/edit-profile.vue:330", "上传头像错误:", error);
            common_vendor.index.showToast({
              title: "上传失败，请稍后重试",
              icon: "none"
            });
        ...
        common_vendor.index.__f__("error", "at pages/profile/edit-profile.vue:341", "上传头像错误:", error);
        common_vendor.index.hideLoading();
        common_vendor.index.showToast({
          title: "网络错误，请稍后重试",
          icon: "none"
    ...
    const selectSchool = () => {
      common_vendor.index.__f__("log", "at pages/profile/edit-profile.vue:352", "选择学院");
    };
    const selectGrade = () => {
    ...
    const updateSkillsToServer = async () => {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pages/profile/edit-profile.vue:434", "更新技能错误:", error);
        common_vendor.index.showToast({
          title: "网络错误，请稍后重试",
          icon: "none"
        });
    ...
    const findSkills = () => {
      common_vendor.index.__f__("log", "at pages/profile/edit-profile.vue:446", "查找技能");
    };
    return (_ctx, _cache) => {
      return common_vendor.e({
        ...
        k: common_vendor.t(profileForm.value.grade || "请选择年级"),
        l: common_vendor.o(selectGrade, "96"),
        m: common_vendor.o(addSkillTag, "99"),
        n: common_vendor.f(profileForm.value.skills, (skill, index, i0) => {
          return {
        ...
        o: common_vendor.o(findSkills, "1e"),
        p: profileForm.value.bio,
        q: common_vendor.o(($event) => profileForm.value.bio = $event.detail.value, "7b"),
        r: showSuccess.value
      }, showSuccess.value ? {} : {});
    };
  }
