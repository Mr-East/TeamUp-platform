...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    common_vendor.onLoad((options) => {
      common_vendor.index.__f__("log", "at pages/publish/recruit-teammate.vue:208", "页面加载，接收到的参数：", options);
      ...
      if (isEditParam && options.postId) {
        ...
        common_vendor.index.__f__("log", "at pages/publish/recruit-teammate.vue:216", "进入编辑模式，帖子ID：", editPostId.value);
        if (options.postData) {
          try {
            common_vendor.index.__f__("log", "at pages/publish/recruit-teammate.vue:220", "开始解析帖子数据...");
            common_vendor.index.__f__("log", "at pages/publish/recruit-teammate.vue:221", "postData原始值：", options.postData);
            const decodedData = decodeURIComponent(options.postData);
            common_vendor.index.__f__("log", "at pages/publish/recruit-teammate.vue:223", "解码后的数据：", decodedData);
            const postData = JSON.parse(decodedData);
            common_vendor.index.__f__("log", "at pages/publish/recruit-teammate.vue:225", "解析后的数据：", postData);
            ...
            common_vendor.index.__f__("log", "at pages/publish/recruit-teammate.vue:235", "编辑模式加载成功，表单数据：", formData);
          } catch (e) {
            common_vendor.index.__f__("error", "at pages/publish/recruit-teammate.vue:237", "解析帖子数据失败", e);
          }
        }
      }
    });
        ...
        success: (res) => {
          ...
          common_vendor.index.uploadFile({
            ...
            fail: (err) => {
              common_vendor.index.hideLoading();
              common_vendor.index.showToast({ title: "上传失败", icon: "none" });
              common_vendor.index.__f__("error", "at pages/publish/recruit-teammate.vue:313", "上传失败", err);
            }
          });
        }
      });
    ...
    const submitForm = async () => {
      ...
      try {
        ...
        common_vendor.index.__f__("log", "at pages/publish/recruit-teammate.vue:388", "submitForm - isEditMode:", isEditMode.value, "editPostId:", editPostId.value);
        const isEdit = isEditMode.value && editPostId.value;
        const url = isEdit ? `http://localhost:3000/api/projects/${editPostId.value}` : "http://localhost:3000/api/projects";
        const method = isEdit ? "PUT" : "POST";
        common_vendor.index.__f__("log", "at pages/publish/recruit-teammate.vue:394", "submitForm - isEdit:", isEdit, "url:", url, "method:", method);
        ...
        if (response.data && response.data.success) {
          ...
          common_vendor.index.showToast({ title: ((_a = response.data) == null ? void 0 : _a.message) || (isEdit ? "修改失败" : "发布失败"), icon: "none" });
        }
      } catch (error) {
        common_vendor.index.__f__("error", "at pages/publish/recruit-teammate.vue:426", "操作失败", error);
      ...
      return common_vendor.e({
        ...
        v: common_vendor.f(common_vendor.unref(competitionCategories), (category, index, i0) => {
          return {
            ...
            b: index,
            c: common_vendor.n({
              selected: formData.competitionType === category
            }),
            d: common_vendor.o(($event) => selectCompetition(category), index)
          };
        }),
        w: common_vendor.t(formData.competitionType || "请选择竞赛大类"),
        x: common_vendor.o(closeCompetitionModal, "a6"),
        y: common_vendor.o(() => {
        }, "16"),
        z: common_vendor.o(closeCompetitionModal, "c4")
      } : {}, {
        A: common_vendor.t(formData.competitionType || "请选择赛事类型"),
        B: common_vendor.o(showCompetitionModal, "b6"),
        C: common_vendor.t(formData.deadline || "请选择日期"),
        D: common_vendor.o(onDateChange, "c6"),
        E: !showPeopleInput.value
      }, !showPeopleInput.value ? {
        F: common_vendor.t(formData.people),
        G: peopleOptions,
        H: common_vendor.o(onPeopleChange, "65")
      } : {
        I: formData.people,
        J: common_vendor.o(common_vendor.m(($event) => formData.people = $event.detail.value, {
          number: true
        }), "b1")
      }, {
        K: common_vendor.t(showPeopleInput.value ? "选择" : "输入"),
        L: common_vendor.o(togglePeopleInput, "04"),
        M: formData.verificationRequired,
        N: common_vendor.o(toggleVerification, "df"),
        O: common_vendor.o(submitForm, "48"),
        P: showSuccess.value
      }, showSuccess.value ? {} : {});
    };
  }
