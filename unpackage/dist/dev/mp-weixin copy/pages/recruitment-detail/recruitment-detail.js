...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const fetchComments = async () => {
      ...
      try {
        const token = common_vendor.index.getStorageSync("token");
        common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:341", "开始获取评论，项目ID:", recruitmentId.value);
        common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:342", "Token:", token ? "存在" : "不存在");
        const response = await common_vendor.index.request({
          url: `http://localhost:3000/api/projects/${recruitmentId.value}/comments`,
          method: "GET",
          header: token ? { "Authorization": `Bearer ${token}` } : {}
        });
        common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:349", "评论响应:", response);
        if (response.data && response.data.success) {
          const commentsData = response.data.data || [];
          common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:354", "原始评论数据长度:", commentsData.length);
          common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:355", "原始评论数据:", commentsData);
          if (commentsData.length > 0) {
            common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:358", "第一条评论的用户信息:", commentsData[0].user);
            if (commentsData[0].replies && commentsData[0].replies.length > 0) {
              common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:360", "第一条回复的用户信息:", commentsData[0].replies[0].user);
            }
          }
          const processCommentUser = (comment) => {
            common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:366", "处理评论用户信息:", comment);
            if (comment.User && !comment.user) {
              common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:369", "发现User字段，转换为user字段:", comment.User);
              comment.user = comment.User;
            }
            if (!comment.user) {
              common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:374", "评论无用户信息，设置默认值");
              comment.user = { name: "匿名用户", avatar: defaultAvatar };
              comment.userId = null;
            } else {
              common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:378", "评论用户信息存在:", comment.user);
              if (comment.user.username) {
                common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:381", "使用username字段:", comment.user.username);
                comment.user.name = comment.user.username;
              } else if (comment.user.name) {
                common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:384", "使用name字段:", comment.user.name);
              } else {
                common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:386", "用户名称不存在，设置默认值");
                comment.user.name = "匿名用户";
              }
              if (!comment.user.avatar) {
                common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:390", "评论用户无头像，设置默认值");
                comment.user.avatar = defaultAvatar;
              }
              if (!comment.userId) {
                common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:395", "评论无userId，从user.id设置");
            ...
            if (comment.parent) {
              if (comment.parent.User && !comment.parent.user) {
                common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:407", "父评论发现User字段，转换为user字段:", comment.parent.User);
          ...
          const processComments = (commentList) => {
            return commentList.map((comment) => {
              common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:427", "处理评论:", comment.id, comment.content);
              common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:428", "评论用户信息:", comment.user);
              comment = processCommentUser(comment);
              if (!comment.replies) {
                comment.replies = [];
              } else {
                common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:437", "评论有回复，数量:", comment.replies.length);
                ...
                common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:460", "扁平化后回复数量:", comment.replies.length);
              }
              return comment;
            });
          };
          const processedComments = processComments(commentsData);
          common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:468", "处理后评论数据长度:", processedComments.length);
          common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:469", "处理后评论数据:", processedComments);
          if (processedComments.length > 0) {
            common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:472", "处理后第一条评论的用户信息:", processedComments[0].user);
            if (processedComments[0].replies && processedComments[0].replies.length > 0) {
              common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:474", "处理后第一条回复的用户信息:", processedComments[0].replies[0].user);
            }
          }
          comments.value = processedComments;
          common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:479", "评论列表已更新:", comments.value.length);
        } else {
          common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:481", "获取评论失败:", (_a = response.data) == null ? void 0 : _a.message);
        }
      } catch (err) {
        common_vendor.index.__f__("error", "at pages/recruitment-detail/recruitment-detail.vue:484", "获取评论错误:", err);
      }
    };
    const submitComment = async () => {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pages/recruitment-detail/recruitment-detail.vue:519", "提交评论错误:", err);
    ...
    const submitReply = async () => {
      ...
      try {
        ...
        common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:537", "回复评论ID:", replyToComment.value.id);
        common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:538", "回复内容:", replyContent.value.trim());
        const response = await common_vendor.index.request({
          url: `http://localhost:3000/api/projects/${recruitmentId.value}/comments`,
          method: "POST",
          header: {
        ...
        common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:553", "回复响应:", response.data);
        if (response.data && response.data.success) {
          common_vendor.index.showToast({ title: "回复成功", icon: "success" });
          replyContent.value = "";
          replyToComment.value = null;
          common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:559", "开始重新获取评论");
          await fetchComments();
          common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:562", "重新获取评论完成");
          setTimeout(async () => {
            common_vendor.index.__f__("log", "at pages/recruitment-detail/recruitment-detail.vue:565", "再次获取评论以确保数据更新");
            await fetchComments();
          }, 500);
        } else {
        ...
        common_vendor.index.__f__("error", "at pages/recruitment-detail/recruitment-detail.vue:572", "提交回复错误:", err);
        common_vendor.index.showToast({ title: "网络错误", icon: "none" });
      }
    };
    ...
    const likeComment = async (commentId) => {
      try {
        ...
        common_vendor.index.__f__("error", "at pages/recruitment-detail/recruitment-detail.vue:618", "点赞评论错误:", err);
      }
    };
    ...
    const submitJoinApplication = async (reasonText) => {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pages/recruitment-detail/recruitment-detail.vue:671", "提交申请错误:", err);
        common_vendor.index.showToast({ title: "网络错误", icon: "none" });
      }
    };
    ...
    common_vendor.onLoad((options) => {
      if (options.data) {
        try {
          ...
          common_vendor.index.__f__("error", "at pages/recruitment-detail/recruitment-detail.vue:754", "解析项目数据失败", e);
        }
      }
      if (options.id) {
      ...
      return common_vendor.e({
        ...
        E: common_vendor.f(comments.value, (comment, index, i0) => {
          ...
          return common_vendor.e({
            ...
            c: common_vendor.t(comment.user.name || "匿名用户"),
            ...
            j: common_vendor.f(comment.replies, (reply, rIndex, i1) => {
              ...
              return common_vendor.e({
                ...
                c: common_vendor.t(reply.user.name || "匿名用户"),
                d: reply.parent && reply.parent.user && reply.parent.user.name
              }, reply.parent && reply.parent.user && reply.parent.user.name ? {
        ...
        I: common_vendor.o(cancelReply, "60")
      } : {}, {
        J: !replyToComment.value
      }, !replyToComment.value ? {
        K: common_vendor.o(submitComment, "86"),
        L: newComment.value,
        M: common_vendor.o(($event) => newComment.value = $event.detail.value, "2b")
      } : {
        N: `回复 ${((_b = replyToComment.value.user) == null ? void 0 : _b.name) || "用户"}...`,
        O: common_vendor.o(submitReply, "d7"),
        P: replyContent.value,
        Q: common_vendor.o(($event) => replyContent.value = $event.detail.value, "72")
      }, {
        R: common_vendor.o(sendComment, "49"),
        S: common_vendor.o(showComment, "dd"),
        T: common_vendor.o(sendPrivateMessage, "63"),
        U: common_vendor.t(joinButtonText.value),
        V: joinStatus.value !== "none" ? 1 : "",
        W: common_vendor.o(handleJoin, "f7")
      });
    };
  }
};
